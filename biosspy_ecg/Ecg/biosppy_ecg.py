...
def plot_ecg(nome_file_da_plottare):
    segnale=np.load('../../biosppy_ecg/Wrk/' + nome_file_da_plottare + '.npy')
        ...
        if n_file >=0 and n_file <=10:
            if type(n_campioni)==int:
                if n_campioni > 10000:
                    ...
                    segnale=np.load('../../biosppy_ecg/Wrk/ecg.npy')#Prelevo il vettore ecg salvato
                    #come ecg.npy nella sottocartella Wrk
                    ecg_seno=ecg_add_sin0(segnale,a,fo)
                    np.save('../../biosppy_ecg/Wrk/ecg_seno.npy', ecg_seno)#Salvo il nuovo vettore
    ...
    if type(nu_file)== int:
        if nu_file >=0 and nu_file <=10:
            ...
            if to>t1:
                ...
                np.save('../../biosppy_ecg/Wrk/ecg_zero.npy',z)#Salvo il nuovo vettore
        ...
        if n_file >=0 and n_file <=10:
            if type(n_campioni)==int:
                if n_campioni > 10000:
                    ...
                    np.save('../../biosppy_ecg/Wrk/ecg_noise.npy', ecgn)#Salvo il nuovo vettore
...
file modificato sotto il nome di ecg_distuba, contenuto nella sottocartella Wrk. Il parametro k rappresenta il seme
del generatore. Se k è maggiore di zero, permette di resettare il generatore di numeri random
che determina quali disturbi vengono applicati, e quindi ogni qual volta diamo quel valore in ingresso,
otteniamo sempre gli stessi disturbi. Se k=0, i valori vengono generati in maniera indipendenti tra loro
"""
    ...
    if type(n_file)== int:
        if n_file >=0 and n_file <=10:
            ...
            if random.random()>0.5:
                ...
                segnale=ecg_zero0(segnale,to,t1 )
            ...
            if random.random()>0.5:
                ...
                segnale=ecg_add_sin0(segnale,a,f0)
            ...
            if random.random()>0.5:
                ...
                np.save('../../biosppy_ecg/Wrk/ecg_disturba.npy',segnale)#Salvo il nuovo vettore
