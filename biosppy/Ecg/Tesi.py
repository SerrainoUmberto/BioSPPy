    ...
    if type(num_file)== int:     #controllo ingresso corretto #devo ancora mettere il controllo sui file
        if type(campioni)==int:
            if campioni > 10000:
                ...
                file= 'C:/Users/dontw/OneDrive/Desktop/Tesi/Dati/rec' +str(num_file)+ '.txt'
                x=np.loadtxt(file)     #load del file scelto
                ecg=x[0:campioni]  #scelta della lunghezza dei campioni
                np.save('C:/Users/dontw/OneDrive/Desktop/Tesi/Wrk/ecg.npy', ecg)
...
def plot_ecg(nome_file_da_plottare):
    segnale=np.load('C:/Users/dontw/OneDrive/Desktop/Tesi/Wrk/' + nome_file_da_plottare + '.npy')
    ...
    if type(n_file)== int:     #Protezione da cosa metto in ingresso
        ...
        if type(n_campioni)==int:
            if n_campioni > 10000:
                ...
                segnale=np.load('C:/Users/dontw/OneDrive/Desktop/Tesi/Wrk/ecg.npy')#Prelevo il vettore ecg salvato
                #come ecg.npy nella sottocartella Wrk
                ecg_seno=ecg_add_sin0(segnale,a,fo)
                np.save('C:/Users/dontw/OneDrive/Desktop/Tesi/Wrk/ecg_seno.npy', ecg_seno)#Salvo il nuovo vettore
...
def ecg_zero(nu_file,to,t1):
    if type(nu_file)== int:
        ...
        if to>t1:
            ...
            np.save('C:/Users/dontw/OneDrive/Desktop/Tesi/Wrk/ecg_zero.npy',z)#Salvo il nuovo vettore
    ...
    if type(n_file)== int:     #Protezione da cosa metto in ingresso
        ...
        if type(n_campioni)==int:
            if n_campioni > 10000:
                ...
                np.save('C:/Users/dontw/OneDrive/Desktop/Tesi/Wrk/ecg_noise.npy', ecgn)#Salvo il nuovo vettore
...
def graf_spettro(nome_file_segnale,finestra):
    x=np.load('C:/Users/dontw/OneDrive/Desktop/Tesi/Wrk/' + nome_file_segnale + '.npy')
...
def ecg_disturba(n_file, k):
    if type(n_file)== int:
        ...
        if x>0.5:
            ...
            to=c+(d-c)*random.random()
            e=0
            t1=e+(d-e)*random.random()
        ...
        if x>0.5:
            c=0.001
            d =(Fs/2)-0.01
            fo=c+(d-c)*random.random()
            e=5
            f=(Fs/2)-0.01
            f1=e+(f-e)*random.random()
            if f1<fo:
                f2=f1
                f1=fo
                fo=f2
            ...
            np.save('C:/Users/dontw/OneDrive/Desktop/Tesi/Wrk/ecg_disturba.npy',ecg_d3)#Salvo il nuovo vettore
